...
public static class RecursiveRenderingProgram {
    public static async Task Execute() {
        ...
        await engine.WriteToStream(CsvOptions.Default(), rows, new StreamWriter(File.OpenWrite("pets.csv")));
        await engine.WriteToStream(CsvOptions.Builder()
            .FullyQualifyNestedProperties()
            .Build(), rows, new StreamWriter(File.OpenWrite("pets2.csv")));
    }
}
